 package sparkrddpackage
  ...
  object RddExercise {
    ...
    def main(args: Array[String]): Unit ={
      ...
      /*val seniorityDaysopenRdd = helpdeskData.map(x => (x.split(",")(2),x.split(",")(8))).filter(x => x._1 == "1 - Junior") //.collect().take(5).foreach(println)//map(x => (x,(x._2.sum,x._2.count)).//reduceByKey(_+_).collect().foreach(println)
      val count4 = seniorityDaysopenRdd.count().println()*/
      val seniorityDaysOpenCount = helpdeskRows.filter(_.requestorSeniority=="1 - Junior").map(_.daysOpen).count()
      println("The count: "+seniorityDaysOpenCount)
      val seniorityDaysOpenSum = helpdeskRows.filter(_.requestorSeniority=="1 - Junior").map(_.daysOpen).sum()
      ...
      //val a = helpdeskRows.filter(_.requestorSeniority=="1 - Junior").map(x => (x.requestorSeniority,x.daysOpen)).mapValues(x => (x,1)).reduce.collect().take(5).foreach(println)
      //using combineByKey
      ...
      val senirityDaysOpenAverage = seniorityDaysOpenTuple.combineByKey((v) => (v, 1),
        ...
        ((acc1: (Int, Int), acc2: (Int, Int)) => (acc1._1+acc2._1, acc1._2+acc2._2)))//.collect().take(5).foreach(println)
      val result = senirityDaysOpenAverage.map{ case (key, value) => (key,value._1 / value._2.toFloat) }.sortByKey().take(5).foreach(println)//.collect().foreach(println)
